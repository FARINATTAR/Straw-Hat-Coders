                    ...
                    {(['red', 'orange', 'yellow', 'green']).map(lv => {
                        ...
                        return (
                            <button key={lv} onClick={() => setLevelFilter(active ? 'all' : lv)}
                                className={`flex items-center gap-1.5 pl-2.5 pr-3 py-1.5 rounded-full text-[11px] font-semibold transition-all cursor-pointer border ${active ? `${s.scoreBg} border-current` : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                                    }`}>
